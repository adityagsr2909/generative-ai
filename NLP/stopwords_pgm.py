...
globally recognised today. Yet we lack the self-confidence to see ourselves as a developed nation, 
...
world, no one will respect us. Only strength respects strength. We must be strong not only as a military 
power but also as an economic power. Both must go hand-in-hand. My good fortune was to have worked 
with three great minds. Dr.Vikram Sarabhai, of the Dept. of Space, Professor Satish Dhawan, 
...
for sentence in nltk.sent_tokenize(corpus):
    ...
    non_stop_words += [word for word in words if word not in stopwordsList]
    words = [wn_lemmatizer.lemmatize(word) for word in words if word not in stopwordsList]
    corpus_lemm.append(" ".join(words))

print("Corpus post StopWords = ", corpus_lemm)

#- Stemming
for word in non_stop_words:
    print(f"{word} :: {port_stemmer.stem(word)} :: {snowball_stemmer.stem(word)} :: {wn_lemmatizer.lemmatize(word, pos='v')}")
